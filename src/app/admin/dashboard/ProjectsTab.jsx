...
export default function ProjectsTab() {
  ...
  const handleSubmit = async () => {
    ...
    setTitle(""); setCategory(""); setDescription(""); setIcon("");
    fetchProjects();
  };
        ...
        {projects.map((project) => (
          <div key={project._id} className="bg-zinc-900 p-4 rounded-xl">
            ...
            <div className="flex gap-4 mt-3">
              <button onClick={() => handleEdit(project)} className="text-yellow-400 text-sm">Edit</button>
              <button onClick={() => handleDelete(project._id)} className="text-red-400 text-sm">Delete</button>
            </div>
          </div>
        ))}
      </div>
